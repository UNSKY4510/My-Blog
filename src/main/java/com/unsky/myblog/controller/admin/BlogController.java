...
public class BlogController {
    ...
    public Result delete(@RequestBody Integer[] ids){
        ...
        if (blogService.deleteBatch(ids)){
            for (int i = 0; i < ids.length; i++) {
                redisUtil.del("blog_id:"+ids[i]);
                redisUtil.zRemove("Blog_Views",ids[i].toString());
                redisUtil.lRemove("Blog_Times",0,ids[i]);
            }
    ...
    public Result save(@RequestParam("blogTitle") String blogTitle,
        ...
        if ("success".equals(saveBlogResult)) {
            System.out.println("成功添加博客,ID:"+blog.getBlogId());
            // 添加到blog的浏览量的Sorted Set
            redisUtil.zAdd("Blog_Views",blog.getBlogId().toString(),0);

            // 添加到blog的发布时间List
            redisUtil.lSet("Blog_Time",blog.getBlogId());

            redisUtil.set("blog_id:"+blog.getBlogId(),0);
