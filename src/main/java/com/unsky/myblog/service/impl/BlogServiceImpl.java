...
public class BlogServiceImpl implements BlogService {
    ...
    public List<SimpleBlogListVO> getBlogListForIndexPage(int type) {
        ...
        if (type == 0){
            Set<Object> blog_ids = redisUtil.zReverseRange("Blog_Views", 0, 8);
            for (Object blog_id: blog_ids) {
                blogs.add(blogMapper.findBlogListByType(blog_id));
            }
        }
        if (type == 1){
            List<Object> blog_ids = redisUtil.lGet("Blog_Time", 0, 8);
            for (Object blog_id: blog_ids) {
                blogs.add(blogMapper.findBlogListByType(blog_id));
            }
        }
